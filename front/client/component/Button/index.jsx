...
const Button = (props) => {
  ...
  if(url==undefined){
    return (
      ...
      onClick={()=>func()} bgColor={bgColor()} ftColor={ftColor()} size={padding()} border={borderColor()} hover={hover()}>
        {icon && <i>{icon()}</i>}
        <span>{value}</span>
      </StyledButton>
    );
    ...
    return(
      <Link href={url}>
        <StyledAnchor 
        style={{marginTop:mt(),marginLeft:ml(),marginBottom:mb(), marginRight:mr()}}
        bgColor={bgColor()} ftColor={ftColor()} size={padding()} border={borderColor()} hover={hover()}>
        {icon && <i>{icon()}</i>}
        <span>{value}</span>
        </StyledAnchor>
      </Link>
